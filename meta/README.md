...
|[`XrCompositorNA`](Samples/XrSamples/XrCompositorNA/)   |`XR_KHR_composition_layer_cube`, `XR_KHR_composition_layer_cylinder`, `XR_KHR_composition_layer_equirect2`, `XR_FB_foveation` |All Meta Quest devices |Single file `C` sample
...
|[`XrHandsCtrl`](Samples/XrSamples/XrHandsCtrl/)         |`XR_META_detached_controllers`,`XR_META_simultaneous_hands_and_controllers`|Meta Quest 3 and later|
...
|[`XrHandWMM`](Samples/XrSamples/XrHandWMM/)      |`XR_META_hand_tracking_wide_motion_mode`|Meta Quest 3 and later|
...
|[`XrPtOcclusion`](Samples/XrSamples/XrPtOcclusion/)        |`XR_META_envionment_depth`|Meta Quest 3 and later|
...
Build all Meta samples from `Samples/` directory:

```bash
cd Samples

# Build all Meta samples - Debug
./gradlew.bat assembleAllDebug

# Build all Meta samples - Release
...
```bash
./gradlew.bat :XrPassthrough:assembleDebug
./gradlew.bat :XrHandsFB:assembleDebug
./gradlew.bat :XrSpatialAnchor:assembleDebug
```
...
Enable Quest system property for experimental features:
...
1. Open `Samples/build.gradle` in Android Studio
2. Or open individual sample: `Samples/XrSamples/XrHandsFB/Projects/Android/build.gradle`
3. Click Run in the toolbar after build completes
