...
private:
    void CreateInstance() {
        ...
        strncpy(AI.applicationName, "OpenXR Tutorial Chapter 6", XR_MAX_APPLICATION_NAME_SIZE);
...
#define CUBE_FACE(V1, V2, V3, V4, V5, V6) vertexPositions[V1], vertexPositions[V2], vertexPositions[V3], vertexPositions[V4], vertexPositions[V5], vertexPositions[V6],
        ...
        pipelineCI.viewMask = 0b11;  // Multiview: enable rendering to both views
    ...
    void CreateSwapchains() {
        ...
        // Multiview: Create ONE swapchain with arraySize = viewCount (instead of per-view swapchains)
        uint32_t viewCount = static_cast<uint32_t>(m_viewConfigurationViews.size());
        m_colorSwapchainInfos.resize(1);
        m_depthSwapchainInfos.resize(1);

        SwapchainInfo &colorSwapchainInfo = m_colorSwapchainInfos[0];
        SwapchainInfo &depthSwapchainInfo = m_depthSwapchainInfos[0];

        // Fill out an XrSwapchainCreateInfo structure and create an XrSwapchain.
        // Color swapchain with arraySize = viewCount for multiview.
        XrSwapchainCreateInfo swapchainCI{XR_TYPE_SWAPCHAIN_CREATE_INFO};
        swapchainCI.createFlags = 0;
        swapchainCI.usageFlags = XR_SWAPCHAIN_USAGE_SAMPLED_BIT | XR_SWAPCHAIN_USAGE_COLOR_ATTACHMENT_BIT;
        swapchainCI.format = m_graphicsAPI->SelectColorSwapchainFormat(formats);
        swapchainCI.sampleCount = m_viewConfigurationViews[0].recommendedSwapchainSampleCount;
        swapchainCI.width = m_viewConfigurationViews[0].recommendedImageRectWidth;
        swapchainCI.height = m_viewConfigurationViews[0].recommendedImageRectHeight;
        swapchainCI.faceCount = 1;
        swapchainCI.arraySize = viewCount;  // Multiview: array for all views
        ...
        // Depth swapchain with arraySize = viewCount for multiview.
        swapchainCI.createFlags = 0;
        ...
        // Get the number of images in the swapchains
        ...
        // Multiview: Create 2D_ARRAY image views that cover all layers
        for (uint32_t j = 0; j < colorSwapchainImageCount; j++) {
            ...
            imageViewCI.layerCount = viewCount;  // All views in one array
    ...
    void DestroySwapchains() {
        // Multiview: Only ONE swapchain to destroy (index 0)
        SwapchainInfo &colorSwapchainInfo = m_colorSwapchainInfos[0];
        SwapchainInfo &depthSwapchainInfo = m_depthSwapchainInfos[0];
    ...
    void RenderCuboid(XrPosef pose, XrVector3f scale, XrVector3f color) {
        ...
        // Multiview: compute modelViewProj for BOTH views
        for (uint32_t v = 0; v < 2; v++) {
            XrMatrix4x4f_Multiply(&cameraConstants.modelViewProj[v], &cameraConstants.viewProj[v], &cameraConstants.model);
        }
    ...
    bool RenderLayer(RenderLayerInfo &renderLayerInfo) {
        ...
        // Resize the layer projection views to match the view count.
        renderLayerInfo.layerProjectionViews.resize(viewCount, {XR_TYPE_COMPOSITION_LAYER_PROJECTION_VIEW});
#if XR_DOCS_CHAPTER_VERSION == XR_DOCS_CHAPTER_5_2
        renderLayerInfo.layerDepthInfos.resize(viewCount, {XR_TYPE_COMPOSITION_LAYER_DEPTH_INFO_KHR});
#endif

        // Multiview: Single swapchain for all views
        SwapchainInfo &colorSwapchainInfo = m_colorSwapchainInfos[0];
        SwapchainInfo &depthSwapchainInfo = m_depthSwapchainInfos[0];

        // Acquire and wait for an image from the swapchains ONCE (not per view)
        ...
        // Get the width and height (same for all views in multiview)
        ...
        // Fill out the XrCompositionLayerProjectionView for EACH view
        // Each view references the SAME swapchain but different imageArrayIndex
        for (uint32_t i = 0; i < viewCount; i++) {
            ...
            renderLayerInfo.layerProjectionViews[i].subImage.imageArrayIndex = i;  // Multiview: each view uses different array layer
#if XR_DOCS_CHAPTER_VERSION == XR_DOCS_CHAPTER_5_2
            ...
            renderLayerInfo.layerDepthInfos[i].subImage.imageArrayIndex = i;  // Multiview: each view uses different array layer
        ...
        // Multiview: Compute view-projection for ALL views upfront
        for (uint32_t i = 0; i < viewCount; i++) {
            XrMatrix4x4f proj;
        ...
        // Rendering - ONCE for all views (multiview renders both views with single draw)
        ...
        // Give the swapchain image back to OpenXR ONCE
